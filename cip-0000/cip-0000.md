...
Additionally, many ideas have been brought forward for changing Global Synchronizer that have been rejected for various reasons.
...
After investigating past work, the best way to proceed is by posting about the new idea to the Global Synchronizer development mailing list.
...
Asking the Global Synchronizer community first if an idea is original helps prevent too much time being spent on something that is guaranteed to be rejected based on prior discussions (searching the internet does not always do the trick).
It also helps to make sure the idea is applicable to the entire community and not just the author. Just because an idea sounds good to the author does not mean it will work for most people in most areas where Global Synchronizer is used.

Once the champion has asked the Global Synchronizer community as to whether an idea has any chance of acceptance, a draft CIP should be presented to the Global Synchronizer development mailing list].
...
Following a discussion, the proposal should be submitted to the CIPs git repository] as a pull request.
This draft must be written in CIP style as described below, and named with an alias such as "CIP-johndoe-infiniteGlobal Synchronizers" until an editor has assigned it a CIP number (authors MUST NOT self-assign CIP numbers).
...
When the CIP draft is complete, a CIP editor will assign the CIP a number, label it as Standards Track, Informational, or Process, and merge the pull request to the CIPs git repository.
The CIP editors will not unreasonably reject a CIP.
Reasons for rejecting CIPs include duplication of effort, disregard for formatting rules, being too unfocused or too broad, being technically unsound, not providing proper motivation or addressing backwards compatibility, or not in keeping with the Global Synchronizer philosophy.
...
The CIP editors subscribe to the Global Synchronizer development mailing list.
...
* The CIP draft must have been sent to the Global Synchronizer development mailing list for discussion.
...
* Specification -- The technical specification should describe the syntax and semantics of any new feature. The specification should be detailed enough to allow competing, interoperable implementations for any of the current Global Synchronizer platforms.

* Motivation -- The motivation is critical for CIPs that want to change the Global Synchronizer protocol. It should clearly explain why the existing protocol is inadequate to address the problem that the CIP solves.
...
* Post-History: <dates of postings to Global Synchronizer mailing list, or link to thread in mailing list archive>
* Requires: <CIP number(s)>
* Replaces: <CIP number>
* Superseded-By: <CIP number>
</pre>

The Layer header (only for Standards Track CIPs) documents which layer of Global Synchronizer the CIP applies to.
See [[CIP-0123.mediawiki|CIP 123]] for definitions of the various CIP layers. Activation of this CIP implies activation of CIP 123.
...
* An Informational CIP describes a Global Synchronizer design issue, or provides general guidelines or information to the Global Synchronizer community, but does not propose a new feature. Informational CIPs do not necessarily represent a Global Synchronizer community consensus or recommendation, so users and implementors are free to ignore Informational CIPs or follow their advice.
* A Process CIP describes a process surrounding Global Synchronizer, or proposes a change to (or an event in) a process. Process CIPs are like Standards Track CIPs but apply to areas other than the Global Synchronizer protocol itself. They may propose an implementation, but not to Global Synchronizer's codebase; they often require community consensus; unlike Informational CIPs, they are more than recommendations, and users are typically not free to ignore them. Examples include procedures, guidelines, changes to the decision-making process, and changes to the tools or environment used in Global Synchronizer development. Any meta-CIP is also considered a Process CIP.
...
<img src="CIP-0002/process.png"></img>
...
A hard-fork CIP requires adoption from the entire Global Synchronizer economy, particularly including those selling desirable goods and services in exchange for Global Synchronizer payments, as well as Global Synchronizer holders who wish to spend or would spend their Global Synchronizers (including selling for other currencies) differently in the event of such a hard-fork. Adoption must be expressed by de facto usage of the hard-fork in practice (ie, not merely expressing public support, although that is a good step to establish agreement before adoption of the CIP). This economic adoption cannot be established merely by a super-majority, except by literally forcing the minority to accept the hard-fork (whether this is viable or not is outside the scope of this document).
...
Software authors are encouraged to publish summaries of what CIPs their software supports to aid in verification of status changes. Good examples of this at the time of writing this CIP, can be observed in [https://github.com/Global Synchronizer/Global Synchronizer/blob/master/doc/CIPs.md Global Synchronizer Core's doc/CIPs.md file] as well as [https://github.com/Global Synchronizer-wallet/Global Synchronizer-wallet/blob/master/wallet/README.specs.md Global Synchronizer Wallet for Android's wallet/README.specs.md file].
...
* CIP 1 defines an ambiguous criteria for the Status field of CIPs, which is often a source of confusion. As a result, many CIPs with significant real-world use have been left as Draft or Proposed status longer than appropriate. By giving objective criteria to judge the progression of CIPs, this proposal aims to help keep the Status accurate and up-to-date.

How is the entire Global Synchronizer economy defined by people selling goods/services and holders?

* For Global Synchronizer to function as a currency, it must be accepted as payment. Global Synchronizers have no value if you cannot acquire anything in exchange for them. If everyone accepting such payments requires a particular set of consensus rules, "Global Synchronizers" are de facto defined by that set of rules - this is already the case today. If those consensus rules are expected to broaden (as with a hard-fork), those merchants need to accept payments made under the new set of rules, or they will reject "Global Synchronizers" as invalid. Holders are relevant to the degree in that they choose the merchants they wish to spend their Global Synchronizers with, and could also as a whole decide to sell under one set of consensus rules or the other, thus flooding the market with Global Synchronizers and crashing the price.
...
For example, the link for CIP 1 will be https://github.com/Global Synchronizer/CIPs/wiki/Comments:CIP-0001 .
...
For example, the preamble to CIP 1 might be updated to include the line:
    ...
    Comments-URI: https://github.com/Global Synchronizer/CIPs/wiki/Comments:CIP-0001
