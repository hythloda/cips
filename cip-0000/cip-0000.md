...
After investigating past work, the best way to proceed is by posting about the new idea to the [https://lists.sync.global/g/cip-discuss](cip-discuss mailing list).
...
Once the champion has asked the Global Synchronizer community whether an idea has any chance of acceptance, a draft CIP should be presented to the [https://lists.sync.global/g/cip-discuss](cip-discuss mailing list).
...
This draft must be written in CIP style as described below, and named with an alias such as "CIP-johndoe-Better-Global Synchronizer" until an editor has assigned it a CIP number (authors MUST NOT self-assign CIP numbers).
...
When the CIP draft is complete, a CIP editor will assign the CIP a number, label it as Standards Track, Governance, Tokenomics, Process or Informational, and merge the pull request to the [https://github.com/global-synchronizer-foundation/cips](cips git repository).
...
The CIP author may update the draft as necessary in the [https://github.com/global-synchronizer-foundation/cips](cips git repository). Updates to drafts should also be submitted by the author as pull requests.
